...
public void reInsertBoard(BoardBean bBean){
		...
		try {
			...
			sql = "update game set re_seq = re_seq+1 where re_ref=? and re_seq > ?";
			pstmt =con.prepareStatement(sql);
	...
	public int updateBoard(BoardBean bBean){
		...
		try {
			con = getConnection();
			sql = "select passwd from game where num = ?";
			pstmt = con.prepareStatement(sql);
			...
			if(rs.next()){
				...
				if(bBean.getPasswd().equals(rs.getString("passwd"))){
					...
					sql = "update game set subject=?, content=? where num = ?";
					pstmt = con.prepareStatement(sql);
		...
		try {
			...
			if(rs.next()){//메게변수로 전달받은 글번호에 해당하는 삭제할 글의 비밀번호가 검색 되었다면
				...
				if(passwd.equals(rs.getString("passwd"))){
					...
					sql = "delete from game where num = ?";
					pstmt = con.prepareStatement(sql);
					pstmt.setInt(1, num);
	...
	public BoardBean getBoardBean(int num){
		...
		try {
			...
			sql = "select * from game where num = ?";
	...
	public void updateReadCount(int num){
		...
		try {
			...
			sql = "update game set readcount=readcount+1 where num=?";
	...
	public List<BoardBean> getBoardList(int startRow, int pageSize){
		...
		try {
			...
			sql = "select * from game order by re_ref desc, re_seq asc limit ?,?";
	...
	public void insertBoard(BoardBean bBean){
		...
		try {
			...
			pstmt = con.prepareStatement(sql);
		 	...
		 	//insert SQL 문만들기
		 	sql = "insert into game (num,name,passwd,"
